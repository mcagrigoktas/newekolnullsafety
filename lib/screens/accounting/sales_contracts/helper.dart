...
class SalesContractHelper {
  SalesContractHelper._();
  static Future<bool> openLabelSettingsDialog() async {
    final _formKey = GlobalKey<FormState>();
    final _existingData = AppVar.appBloc.schoolInfoForManagerService!.singleData!.salesContractLabelInfoList();
    Map _newData = {};
    ...
    await QuickPage.openFullPage(
      ...
      onSave: () async {
        if (_formKey.currentState!.checkAndSave()) {
          await AccountingService.saveSalesContractLabelSettings(_newData).then((value) {
            OverAlert.saveSuc();
          }).catchError((_) {
            OverAlert.saveErr();
          });
